...
class AuthController(
    ...
    fun getUser(
        ...
        if (token != null) {
            ...
            if (token == "test-token" && isDevProfile) {
                logger.info { "TestUser authentication successful (dev profile)" }
                val testUser = mapOf(
                    "id" to 0L,
                    "name" to "Test User",
    ...
    fun oauthCallback(@AuthenticationPrincipal principal: OAuth2User?): ResponseEntity<String> {
        ...
        if (principal != null) {
            ...
            // Redirect to frontend with token and safely-serialized user JSON
            val userMap = mapOf(
                "id" to user.id,
                "name" to user.name,
                "email" to user.email,
    ...
    fun testOAuth(): ResponseEntity<Any> {
        if (!isDevProfile) {
